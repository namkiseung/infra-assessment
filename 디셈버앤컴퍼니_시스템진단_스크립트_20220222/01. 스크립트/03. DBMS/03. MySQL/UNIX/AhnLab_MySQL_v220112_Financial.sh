...
HOSTNAME=`hostname`
DATE=`date +%Y-%m-%d`
FILENAME=MYSQL_${HOSTNAME}_${DATE}

echo "MySQL 계정ID를 입력하시오!"
read MYSQL_USERNAME
echo "MySQL 비밀번호를 입력하시오!"
read MYSQL_PASSWORD
...
echo "********* AhnLab System Checklist for Mysql DBMS **********"																>> ./$FILENAME.log 2>&1
...
echo "[# 1.1 MySQL Version #]" >>./$FILENAME.log 2>&1
mysql -u $MYSQL_USERNAME -p$MYSQL_PASSWORD mysql -e "SELECT version();" >> ./$FILENAME.log 2>&1

echo "[# 1.2 MySQL 비밀번호가 없는 계정 확인 (MySQL <= 5.6) #]" >>./$FILENAME.log 2>&1
mysql -u $MYSQL_USERNAME -p$MYSQL_PASSWORD mysql -e "SELECT user, host FROM mysql.user WHERE password = '';" >>./$FILENAME.log 2>&1

echo "[# 1.3 MySQL 비밀번호가 없는 계정 확인 (MySQL >= 5.7) #]" >>./$FILENAME.log 2>&1
...
echo "1. 1.1의 MySQL 버전이 5.6 이하이고, 1.2에 계정이 존재하지 않는 경우 양호" >>./$FILENAME.log 2>&1
echo "2. 1.1의 MySQL 버전이 5.7 이상이고, 1.3에 계정이 존재하지 않는 경우 양호" >>./$FILENAME.log 2>&1
echo "* 취약 : 비밀번호 복잡도를 만족하지 않거나 취약하다고 판단되는 계정 비밀번호가 존재할 경우" >>./$FILENAME.log 2>&1
echo "1. 1.1의 MySQL 버전이 5.6 이하이고, 1.2에 계정이 존재하는 경우 취약" >>./$FILENAME.log 2>&1
echo "2. 1.1의 MySQL 버전이 5.7 이상이고, 1.3에 계정이 존재하는 경우 취약" >>./$FILENAME.log 2>&1
echo 
...
echo "참고 : MySQL에서 지원하는 암호화 함수"																					>> ./$FILENAME.log 2>&1
...
echo "[# 1.2 MySQL 자체기능(MySQL >= 8.0.19) #]" >>./$FILENAME.log 2>&1
mysql -u $MYSQL_USERNAME -p$MYSQL_PASSWORD mysql -e "SELECT user, host, User_attributes FROM mysql.user;" >>./$FILENAME.log 2>&1
echo "failed_login_attempts 값이 5초과로 되어있거나 password_lock_time_days 값이 지나치게 작게 설정되어 있는 경우 취약"																	>> ./$FILENAME.log 2>&1
echo 																>> ./$FILENAME.log 2>&1

echo "[# 1.3 인터뷰 (MySQL < 8.0.19) #]" >>./$FILENAME.log 2>&1
...
echo "비밀번호 정책 확인"																										>> ./$FILENAME.log 2>&1
	mysql -u $MYSQL_USERNAME -p$MYSQL_PASSWORD mysql -e "SHOW VARIABLES LIKE 'validate_password%';" -t							>> ./$FILENAME.log 2>&1
echo 																									>> ./$FILENAME.log 2>&1
echo "위에 결과값이 나오지 않으면 플러그인, 장비, 써드파티 솔루션 등을 통해 비밀번호 복잡도 준수를 강제하고 있는지 여부 확인"																										>> ./$FILENAME.log 2>&1
echo																															>> ./$FILENAME.log 2>&1
...
echo 																														>> ./$FILENAME.log 2>&1
echo "참고"																														>> ./$FILENAME.log 2>&1
echo "validate_password_check_user_name : 패스워드에 userid의 사용 여부 확인"													>> ./$FILENAME.log 2>&1
echo "validate_password_dictionary_file : 패스워드에 dictionary file 검사"														>> ./$FILENAME.log 2>&1
echo "validate_password_length : 패스워드 길이						"															>> ./$FILENAME.log 2>&1
echo "validate_password_mixed_case_count : 패스워드에 사용 가능한 대소문자 최소 갯수 설정"										>> ./$FILENAME.log 2>&1
echo "validate_password_number_count : 패스워드에 사용 가능한 최소 숫자 갯수 설정"												>> ./$FILENAME.log 2>&1
echo "validate_password_policy : 패스워드정책의 수준 설정"																		>> ./$FILENAME.log 2>&1
echo "validate_password_special_char_count : 패스워드에 사용 가능한 특수문자의 최소 갯수 설정"									>> ./$FILENAME.log 2>&1
echo "##### END #####"                                              	>> $FILENAME.log 2>&1
echo																															>> ./$FILENAME.log 2>&1
echo "====================================================================================================="					>> ./$FILENAME.log 2>&1
echo																															>> ./$FILENAME.log 2>&1
...
echo "마지막 비밀번호 변경일자 확인"																									>> ./$FILENAME.log 2>&1
	mysql -u $MYSQL_USERNAME -p$MYSQL_PASSWORD mysql -e "SELECT USER, HOST, password_last_changed from mysql.user;;" -t					>> ./$FILENAME.log 2>&1
echo																															>> ./$FILENAME.log 2>&1
...
echo "버그 패치 된 릴리즈 사이트 : http://downloads.mysql.com/archives.php"														>> ./$FILENAME.log 2>&1
...
echo "##### START #####"                                              	>> $FILENAME.log 2>&1
echo  >> $FILENAME.log 2>&1
echo "* (v8.0이상) 데이터베이스 자체기능(password_history, password_resue_interval) 값 확인"																											>> ./$FILENAME.log 2>&1
	mysql -u $MYSQL_USERNAME -p$MYSQL_PASSWORD mysql -e "SHOW VARIABLES LIKE 'password_history';" -t											>> ./$FILENAME.log 2>&1
	mysql -u $MYSQL_USERNAME -p$MYSQL_PASSWORD mysql -e "SHOW VARIABLES LIKE 'password_reuse_interval';" -t											>> ./$FILENAME.log 2>&1
echo "- password_history값이 0이거나 password_resue_interval값이 0일 경우  취약"																														>> ./$FILENAME.log 2>&1
echo "v8.0 미만인 경우, 플러그인, 써드파티 솔루션, 내부 정책 등을 통해 이전 비밀번호 재사용을 제한하고 있는지 확인"																														>> ./$FILENAME.log 2>&1
...
echo "참고"																													>> ./$FILENAME.log 2>&1
echo "※ WITH GRANT OPTION : GRANT받은 권한 또는 역할을 타 사용자에게 부여할수 있는 옵션으로 MySQL에서는 IS_GRANTABLE 칼럼을 통해 설정 가능"																															>> ./$FILENAME.log 2>&1
echo "##### END #####"                                              	>> $FILENAME.log 2>&1
echo																															>> ./$FILENAME.log 2>&1
echo "====================================================================================================="					>> ./$FILENAME.log 2>&1
echo																															>> ./$FILENAME.log 2>&1
