...
echo '===================================================================' >> $CREATE_FILE_RESULT 2>&1
echo '■ 기준' >> $CREATE_FILE_RESULT 2>&1
echo 'Tomcat 설정파일 퍼미션이 WAS전용 계정 소유이며 600으로 설정되어 있는 경우 양호' >> $CREATE_FILE_RESULT 2>&1
echo '' >> $CREATE_FILE_RESULT 2>&1
echo '■ 현황' >> $CREATE_FILE_RESULT 2>&1
...
echo '※ Tomcat conf 디렉터리 내부의 설정파일 퍼미션을 600 이하로 설정 권고' >> $CREATE_FILE_RESULT 2>&1
echo ' ' >> $CREATE_FILE_RESULT 2>&1
...
echo 'Tomcat 9.0.x	9.0.14' >> $CREATE_FILE_RESULT 2>&1
echo 'Tomcat 8.5.x	8.5.37' >> $CREATE_FILE_RESULT 2>&1
echo 'Tomcat 8.0.x	8.0.53' >> $CREATE_FILE_RESULT 2>&1
echo 'Tomcat 7.0.x	7.0.92' >> $CREATE_FILE_RESULT 2>&1
echo 'Tomcat 6.0.x	6.0.53' >> $CREATE_FILE_RESULT 2>&1
echo 'Tomcat 5.5버전 이하는 기술지원 완료로 인해 패치 지원 안함' >> $CREATE_FILE_RESULT 2>&1
echo ' ' >> $CREATE_FILE_RESULT 2>&1
