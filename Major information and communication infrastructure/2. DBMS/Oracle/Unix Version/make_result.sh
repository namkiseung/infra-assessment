...
echo "*****************************************************************************" >> $_RET_TXT 
echo "        Copyright 2017 think Co. Ltd. All Rights Reserved. Ver$Version	   " >> $_RET_TXT 
...
echo "[기준]" >> $_RET_TXT
echo "- UNIX 서버 : N/A" >> $_RET_TXT
echo "- Windows 서버 : 서버취약점 점검 항목 결과 반영" >> $_RET_TXT
echo " " >> $_RET_TXT
echo " " >> $_RET_TXT
...
_CONTROL=`cat $_TMP_DIR/2.7.control.txt | grep / | awk -F, '{print $1}'`
...
_DATA_FILE=`cat $_TMP_DIR/2.7.datafile.txt | grep / | awk -F, '{print $1}'`
...
_LOG_FILE=`cat $_TMP_DIR/2.7.logfile.txt | grep / | awk -F, '{print $1}'`
...
_SP_FILE=`cat $_TMP_DIR/2.7.spfile.txt | grep / | awk -F, '{print $1}'`
